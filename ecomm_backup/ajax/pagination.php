<?php require_once("../adminpanel/lib/helper.php");
	...
	$where_append = "";
	if(isset($_GET['brand'])){
		$brand_id = base64_decode(base64_decode(base64_decode(base64_decode($_GET['brand']))));
		$where_append.= " AND brand_id = '$brand_id'";
	}
	if(isset($_GET['design'])){
		$design_id = base64_decode(base64_decode(base64_decode(base64_decode($_GET['design']))));
		$where_append.= " AND design_id = '$design_id'";
	}
	if(isset($_GET['device'])){
		$device_id = base64_decode(base64_decode(base64_decode(base64_decode($_GET['device']))));
		$where_append.= " AND device_id = '$device_id'";
	}
	//get total number of records from database
	$results= $obj->select("product_id","Products", "product_status='1' $where_append");
	...
	$results= $obj->select("*","Products", "product_status='1' $where_append ORDER BY product_id ASC LIMIT $page_position ,$item_per_page");
