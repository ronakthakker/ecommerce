<?php
	...
	<?php
		...
		$product_arr = $obj->select("P.product_id, P.product_title, C.category_name, M.material_name","product_master AS P INNER JOIN material_master AS M ON P.material_id = M.material_id INNER JOIN sub_category_master AS SC ON SC.sub_category_id = SC.sub_category_category_id INNER JOIN category_master AS C ON SC.sub_category_category_id = C.category_id","P.product_status='1' AND M.material_status='1' AND SC.sub_category_status='1' AND C.category_status='1' ORDER BY product_id DESC  LIMIT $start, $limit");
